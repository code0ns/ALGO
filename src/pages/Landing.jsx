...
import { ShieldCheck, ArrowRight, Settings2, BarChart3, BookOpen, HelpCircle } from 'lucide-react';
...
export default function Landing() {
  ...
  const projection = useMemo(
    () => projectGrowth({ monthlyContribution: contribution, riskKey: risk, years, startingValue: startBalance }),
    ...
    <div className="landing-container" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <header className="landing-header">
        ...
        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
          ...
          <button className="btn-ghost" onClick={() => navigate('/faq')}>
      ...
      <main className="container" style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '64px 24px 32px' }}>
          ...
          <div className="card" style={{ padding: '24px' }}>
            <div className="projection-controls">
              <div>
                ...
                <input type="range" min="0" max="50000" step="500" value={startBalance} onChange={(e) => setStartBalance(Number(e.target.value))} className="slider" />
              ...
              <div>
                ...
                <input type="range" min="50" max="3000" step="50" value={contribution} onChange={(e) => setContribution(Number(e.target.value))} className="slider" />
              ...
              <div>
                ...
                <input type="range" min="1" max="30" step="1" value={years} onChange={(e) => setYears(Number(e.target.value))} className="slider" />
              ...
              <div>
                ...
                <div className="risk-tabs" style={{ marginTop: '6px' }}>
                  {Object.keys(RISK_PRESETS).map((k) => (
                    <button key={k} className={`risk-tab ${risk === k ? 'active' : ''}`} onClick={() => setRisk(k)}>{k}</button>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div style={{ height: '220px', marginTop: '16px' }}>
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={projection} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
                  ...
                  <XAxis dataKey="month" tick={{ fontSize: 11, fill: 'var(--text-muted)' }} axisLine={false} tickLine={false} interval={Math.max(1, Math.floor(projection.length / 6))} />
                  <YAxis hide />
        ...
        <section style={{ marginBottom: '64px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '24px', alignItems: 'start' }}>
            <PricingCard />
            <div>
              <h3 style={{ fontSize: '20px', marginBottom: '12px' }}>How your money is protected</h3>
              <TrustBadges />
            </div>
          </div>
        </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '32px', borderTop: '1px solid var(--border-color)', paddingTop: '48px' }}>
          <div>
            <div style={{ marginBottom: '16px', color: 'var(--text-secondary)' }}><Settings2 size={24} /></div>
            <h3 style={{ marginBottom: '8px', fontSize: '18px' }}>Set it and forget it</h3>
            <p className="text-secondary" style={{ fontSize: '15px' }}>Define your monthly allocation and risk parameters once. The system handles the rest.</p>
          </div>
          <div>
            <div style={{ marginBottom: '16px', color: 'var(--text-secondary)' }}><BarChart3 size={24} /></div>
            <h3 style={{ marginBottom: '8px', fontSize: '18px' }}>Transparent reporting</h3>
            <p className="text-secondary" style={{ fontSize: '15px' }}>Clear views of your balance vs benchmark. No complex charts, just steady growth.</p>
          </div>
          <div>
            <div style={{ marginBottom: '16px', color: 'var(--text-secondary)' }}><ShieldCheck size={24} /></div>
            <h3 style={{ marginBottom: '8px', fontSize: '18px' }}>Risk limits enforced</h3>
            <p className="text-secondary" style={{ fontSize: '15px' }}>Strictly enforced limits ensure your exposure never exceeds what you configure.</p>
          </div>
        </div>
      </main>
      ...
      <footer style={{ padding: '32px 24px', textAlign: 'center', borderTop: '1px solid var(--border-color)', marginTop: 'auto' }}>
        <p className="text-muted" style={{ fontSize: '14px' }}>
          ...
          <span style={{ marginLeft: '12px' }}>
            <a href="#" onClick={(e) => { e.preventDefault(); navigate('/faq'); }} style={{ color: 'var(--text-secondary)' }}>FAQ</a>
          </span>
        </p>
      </footer>
    </div>
